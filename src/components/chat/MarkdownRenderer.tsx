...
function highlightCode(code: string, lang: string) {
    ...
    const keywords = /\b(const|let|var|function|return|if|else|for|while|import|export|from|async|await|class|interface|type|public|private|protected|try|catch|finally|def|in|is|not|and|or|print|new|this|super|extends|implements|static|void|null|undefined|true|false)\b/g;
    const strings = /("[^"]*"|'[^']*'|`[^`]*`)/g;
        ...
        <div className="max-w-none break-words text-sm text-foreground/90 leading-relaxed [&_strong]:font-bold [&_em]:italic">
            <ReactMarkdown
                ...
                components={{
                    ...
                    th: ({ node: _node, ...props }) => (
                        <th className="px-4 py-3 text-left font-semibold text-foreground whitespace-nowrap text-sm" {...props} />
                    ),
                    td: ({ node: _node, ...props }) => (
                        <td className="px-4 py-3 align-top text-sm" {...props} />
                    ),
                    p: ({ node: _node, ...props }) => (
                        <p className="text-sm text-foreground/90 leading-relaxed mb-4 last:mb-0" {...props} />
                    ),
                    h1: ({ node: _node, ...props }) => (
                        <h1 className="text-2xl font-bold text-foreground mt-6 mb-3 leading-tight" {...props} />
                    ),
                    h2: ({ node: _node, ...props }) => (
                        <h2 className="text-xl font-bold text-foreground mt-5 mb-3 leading-tight" {...props} />
                    ),
                    h3: ({ node: _node, ...props }) => (
                        <h3 className="text-lg font-semibold text-foreground mt-4 mb-2 leading-snug" {...props} />
                    ),
                    h4: ({ node: _node, ...props }) => (
                        <h4 className="text-base font-semibold text-foreground mt-4 mb-2 leading-snug" {...props} />
                    ),
                    h5: ({ node: _node, ...props }) => (
                        <h5 className="text-sm font-semibold text-foreground mt-3 mb-1.5" {...props} />
                    ),
                    h6: ({ node: _node, ...props }) => (
                        <h6 className="text-sm font-medium text-muted-foreground mt-3 mb-1.5 uppercase tracking-wide" {...props} />
                    ),
                    ul: ({ node: _node, ...props }) => (
                        <ul className="list-disc pl-6 mb-4 space-y-1.5 text-sm text-foreground/90 marker:text-muted-foreground" {...props} />
                    ),
                    ol: ({ node: _node, ...props }) => (
                        <ol className="list-decimal pl-6 mb-4 space-y-1.5 text-sm text-foreground/90 marker:text-muted-foreground" {...props} />
                    ),
                    li: ({ node: _node, ...props }) => (
                        <li className="pl-1 leading-relaxed [&>ul]:mt-1.5 [&>ol]:mt-1.5 [&>ul]:mb-0 [&>ol]:mb-0" {...props} />
                    ),
                    a: ({ node: _node, ...props }) => (
                        <a className="text-blue-400 hover:text-blue-300 hover:underline font-medium cursor-pointer" target="_blank" rel="noopener noreferrer" {...props} />
                    ),
